...
function FeedBrowserViewModel() {
  ...
  self.feedUrl = ko.observable('').extend({
    required: false,
    isValidUrlOrValidLitecoinAdressOrJsonBet: self
  });
  ...
  $.jqlog.debug("displayFeed1"); 
    ...
    for(var i = 0; i < addresses.length; i++) {
      options.push({
        ...
        label: addresses[i][1] + ' (' + addresses[i][2] + ' XLT)'
      });
    ...
    for (var i in feed.counters.bets) {
    	feed.counters.bets[i].wager_quantity = normalizeQuantity(feed.counters.bets[i].wager_quantity) + ' XLT';
    	feed.counters.bets[i].wager_remaining = normalizeQuantity(feed.counters.bets[i].wager_remaining) + ' XLT';
    ...
    var onCounterbetsLoaded = function(data) {
      ...
      if (displayedData.length>0) {
        ...
        for (var b = 0; b < displayedData2.length; b++) {
          ...
          displayedData2[b].volume_str = satoshiToXLT(displayedData2[b].countervolume);
          displayedData2[b].counterwager = satoshiToXLT(displayedData2[b].counterwager);
        }
      }
...
function OpenBetsViewModel() {
  ...
  self.displayOpenBets = function(data) {
    ...
    for (var i=0; i<data.bets.length; i++) {
      ...
      bet.wager_quantity = satoshiToXLT(data.bets[i].wager_quantity);
      bet.wager_remaining = satoshiToXLT(data.bets[i].wager_remaining);
      bet.counterwager_quantity = satoshiToXLT(data.bets[i].counterwager_quantity);
      bet.counterwager_remaining = satoshiToXLT(data.bets[i].counterwager_remaining);
...
function MatchedBetsViewModel() {
  ...
  self.displayMatchedBets = function(data) {
    ...
    var genBetItem = function(data_bet, num_tx, feeds) {
      ...
      bet.wager = satoshiToXLT(bet.wager);
      bet.counterwager = satoshiToXLT(bet.counterwager);
