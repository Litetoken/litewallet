function BalanceHistoryViewModel() {
  ...
  self.availableAssets = !USE_TESTNET ? ko.observableArray(["XLT", "LTC"]) : ko.observableArray(["XLT"]);
  ...
  self.init = function() {
    ...
    failoverAPI("get_owned_assets", { addresses: WALLET.getAddressesList() }, function(data, endpoint) {
      ...
      //^ this way, XLT and LTC stay at the top, but the other assets are alphabetically sorted
    });
  ...
  self.dispDescription = function() {
    ...
    if(self.RAW_TX_TYPE == 'burns') {
      ...
      if(self.DATA['forward_asset'] == 'LTC' || self.DATA['backward_asset'] == 'LTC') {
        desc += " <b>(" + i18n.t("pending LTCpay") + ")</b>";
      }
    } else if(self.RAW_TX_TYPE == 'ltcpays') {
      desc = i18n.t("hist_ltcpay", smartFormat(normalizeQuantity(self.DATA['ltc_amount'])));
