  ...
  var quantityValidator = {
    ...
    validation: {
      validator: function (val, self) {
        ...
        if (self.donationCurrency() == 'XLT') {
          return parseFloat(quantity) <= self.balancesXLT[address];
        } else {
  ...
  self.init = function() {
    ...
    for(var i = 0; i < addresses.length; i++) {
      ...
      options.push({
        ...
        label: addresses[i][1] + ' (' + round(ltcBalance, 2) + ' LTC / ' + round(addresses[i][2], 2) + ' XLT)'
      });
      self.balancesLTC[addresses[i][0]] = ltcBalance;
      self.balancesXLT[addresses[i][0]] = addresses[i][2];
    }
