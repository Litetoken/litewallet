...
var AssetLeaderboardViewModel = CClass.create(function() {
  ...
  self.marketCapTables = ko.observableArray([
    {'base': 'XLT', 'data': ko.observableArray([])},
  ...
  self.init = function(assets) {
    ...
    failoverAPI(self.isLeaderboard ? "get_market_info_leaderboard" : "get_market_info", self.isLeaderboard ? {} : {assets: assets}, function(data, endpoint) {
      ...
      self.showPortfolioIn("XLT"); //causes the table to be generated off of self.marketInfo
    });
  ...
  self.updateMarketInfo = function() {
    ...
    //label XLT marketcap positions
    marketInfo = self.isLeaderboard ? self.marketInfo['xlt'] : self.marketInfo; 
    marketInfo.sort(
      function(l, r) {
        return l['market_cap_in_xlt'] == r['market_cap_in_xlt'] ? 0 : (l['market_cap_in_xlt'] < r['market_cap_in_xlt'] ? 1 : -1)
      }
    );
    for(i=0; i < marketInfo.length; i++) {
      marketInfo[i]['position_xlt'] = i + 1;
    }
    assert(self.marketCapTables()[0]['base'] == 'XLT');
    for(i=0; i < marketInfo.length; i++) {
      if(!marketInfo[i]['price_in_xlt']) continue;
      self.marketCapTables()[0]['data'].push({
        position: marketInfo[i]['position_xlt'],
        ...
        marketCap: marketInfo[i]['market_cap_in_xlt'] ? (smartFormat(marketInfo[i]['market_cap_in_xlt'], 100, 0) + ' XLT') : '',
        price: marketInfo[i]['aggregated_price_as_xlt'] ? (smartFormat(marketInfo[i]['aggregated_price_as_xlt'], 10, 4) + ' XLT') : '',
        ...
        //volume: (marketInfo[i]['24h_ohlc_in_xlt']['vol'] && marketInfo[i]['aggregated_price_in_xlt']) 
        //  ? (smartFormat(marketInfo[i]['24h_ohlc_in_xlt']['vol'] * marketInfo[i]['aggregated_price_in_xlt'], 0, 4) + ' XLT') : '',
        volume: (marketInfo[i]['24h_summary'] && marketInfo[i]['24h_summary']['vol'] && marketInfo[i]['aggregated_price_in_xlt']) 
          ? (smartFormat(marketInfo[i]['24h_summary']['vol'] * marketInfo[i]['aggregated_price_in_xlt'], 10, 4) + ' XLT') : '',
        pctChange: marketInfo[i]['24h_vol_price_change_in_xlt'] ? (smartFormat(marketInfo[i]['24h_vol_price_change_in_xlt'], 0, 2) + ' %') : '',
        pctChangeColorClass: marketInfo[i]['24h_vol_price_change_in_xlt'] > 0 ? 'txt-color-green' : (marketInfo[i]['24h_vol_price_change_in_xlt'] < 0 ? 'txt-color-red' : 'initial'),
        history: marketInfo[i]['7d_history_in_xlt'],

        marketCapRaw: marketInfo[i]['market_cap_in_xlt'],
        priceRaw: marketInfo[i]['aggregated_price_as_xlt'],
        supplyRaw: marketInfo[i]['total_supply'],
        volumeRaw: marketInfo[i]['24h_ohlc_in_xlt'] ? marketInfo[i]['24h_ohlc_in_xlt']['vol'] : 0,
        pctChangeRaw: marketInfo[i]['24h_vol_price_change_in_xlt']
      });
    }
  ...
  self.showPortfolioInXLT = function() {
    self.showPortfolioIn("XLT");
  }
  ...
  self.showPortfolioIn.subscribeChanged(function(newValue, prevValue) {
    ...
    assert(newValue == "XLT" || newValue == "LTC", "Invalid value");
...
AssetLeaderboardViewModel.formulateExtendedAssetInfo = function(asset, hasImage, website) {
  ...
  if(asset == 'XLT' || asset == 'LTC') {
    ...
    var website = asset == 'XLT' ? "http://www.litetokens.co" : "http://www.litecoin.org";
