...
function PendingActionFeedViewModel() {
  ...
  self.remove = function(txHash, category, ltcRefreshSpecialLogic) {
    ...
    if(match) {
      ...
      // send txns (e.g. is a litetokens asset send, or asset issuance, or something the LTC balance refresh
...
PendingActionFeedViewModel.modifyBalancePendingFlag = function(category, data, flagSetting) {
  ...
  if(category == 'burns') {
    ...
    addressObj.getAssetObj("XLT").balanceChangePending(flagSetting);
    ...
    if(assetObj && assetObj.isMine()) {
      ...
      // issuance fee
      updateUnconfirmedBalance(data['source'], 'XLT', -ASSET_CREATION_FEE_XLT * UNIT);
    }
    ...
    updateUnconfirmedBalance(data['source'], 'XLT', data['wager_quantity'] * -1);
    
  } else if (category == 'rps') {

    updateUnconfirmedBalance(data['source'], 'XLT', data['wager'] * -1);
