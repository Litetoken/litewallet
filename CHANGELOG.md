...
**Fixes:**
* Significant performance and reliability improvements (on the liteblockd side)
...
* Added IP-based blocking for betting, RPS and dividend issuance. We are currently blocking people
  with US-based IP addresses from using these features on litewallet.co itself.
...
### v0.9.5 ###
**Enhancements:**
* implemented asset description json and asset pictures, yay (see https://wiki.litetokens.co/w/Enhanced_Asset_Info_in_Litewallet)
...
* hopefully fixed a bug where if litetokens/coutnerwalletd were in the process of processng a block at the same time a client made a request, it would log the client out (saying that the server is not caught up to the blockchain)...this check is more tolerant now

### v0.9.2 ###
**Enhancements:**
* added the ASSET LEADERBOARD functionality, which shows the top 100 litetokens assets (that have actual trade data)* asset market info is now pre-compiled (i.e. calculated and stored) every 10 minutes, due to the addition of the asset leaderboard* added notification pane messages for bets and broadcasts
* servers store chat history now in database, and added get_chat_history() call. 
* because of the above, chat history persists between server restarts (so you still get the newest line even if the litewalletd service needs to be restarted)
...
* fixes for nginx "crash" situation, fix correctly closing unused sockets in litewalletd in one or two places
...
* 0.9.2.1: bug fix for litewalletd not properly rebuilding its app_config collection if the cwd DB version went up
